pipeline {
    ...
    environment { REGISTRY = 'harbor.matchday.internal' }
    stages {
        ...
        stage('Sign')  { steps { sh 'cosign sign --yes ${REGISTRY}/matchday/${SERVICE}:${GIT_COMMIT}' } }
        stage('Push')  { steps { sh 'make push' } }
    }
    post {
        failure { slackSend channel: '#matchday-ci', message: "Build failed: ${env.BUILD_URL}" }
    }
}
