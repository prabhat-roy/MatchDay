...
deny[msg] if {
    ...
    not startswith(container.image, "harbor.matchday.internal/")
    msg := sprintf("image %q must be from harbor.matchday.internal", [container.image])
}
