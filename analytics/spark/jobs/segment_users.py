...
spark = SparkSession.builder.appName("matchday-segment").getOrCreate()
df = spark.read.format("iceberg").load("matchday_mart.user_features")
...
preds.select("user_id","prediction").writeTo("matchday_mart.user_segments").using("iceberg").createOrReplace()
