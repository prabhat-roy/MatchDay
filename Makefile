...
help:
	@echo "MatchDay â€” common targets"
	@echo ""
...
tools-deploy:  ## Deploy every OSS tool registered for MatchDay via ArgoCD
	kubectl apply -f gitops/argocd/applicationsets/all-tools-applicationset.yaml
	argocd appset get matchday-tools --grpc-web

tools-status:  ## Show sync status of every registered tool
	argocd app list -l "project=matchday-tools" --grpc-web
